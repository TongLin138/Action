...
function Add_Repo() {
    ...
    function CreateTemplate() {
        echo '{ "name": "", "url": "", "branch": "", "enable": true, "cronSettings": { "updateTaskList": true, "scriptsPath": "", "scriptsType": ["js"], "whiteList": "", "blackList": "", "autoDisable": false, "addNotify": true, "delNotify": true } }' | jq | yq -y >$tmp_file
        # 插入缩进空格
    ...
    function SaveConf() {
        ...
        if [ $? -eq 0 ]; then
            echo -e "$SUCCESS 已写入至 ${BLUE}${FileSyncConfUser##*/}${PLAIN} 配置文件，你可以在之后执行更新命令 ${BLUE}${UpdateCmd} repo${PLAIN} 来使该配置生效\n"
        else
